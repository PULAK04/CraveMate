...
export const fetchMyCurrentOrder = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      res.status(500).json({
        message: error.response?.data?.message || "Could not fetch current order",
      });
    }
  }
);
...
export const broadcastRiderLocation = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const userId = req.user?._id;
    const { orderId } = req.params;
    ...
    if (
      ...
      return res.status(400).json({ message: "Valid rider location is required" });
    }

    const rider = await Rider.findOne({ userId, isVerified: true });
    if (!rider) {
      return res.status(403).json({ message: "Verified rider profile required" });
    }
